...
| **Bid guidance leans the wrong way, and the page said so backwards** | **Measured 22 Sept, not guessed.** The backtest's median bias is era-dependent and the sign FLIPS: all 97 claims **-0.14pp**, 2024+ **+0.34**, 2025+ **+0.52**, 2026 **+0.51**. Positive means the method quotes ABOVE what the auction paid. `TrackRecord` printed the all-period sign, so it told readers the range "usually quotes below what the auction paid" while the current regime does the opposite — a true figure answering a question nobody asked, the same shape as the reader banner that reported pipeline liveness to somebody pricing a bond. The live ledger agrees far more bluntly: **all three scored predictions missed, every one BELOW the range**. Cause is not a bug — Kenyan yields fell hard through 2025-26 and a range built from past prints lags a falling market; every recorded prediction carries `windowDays: null`, meaning each one fell through the recency ladder to the full-archive fallback that still contains the 17-18pp era. `recentBiasPp` (24-claim window, returns **null** under that, because absence is not zero) now publishes the current lean beside the long-run one whenever they disagree in sign. **The method itself was NOT changed** — widening or re-centring the band is a product call and the owner's. Middle-half coverage is the number to watch: 4/26 in 2026 against the ~50% a calibrated band should give. |
