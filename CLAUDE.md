...
| **Data pipeline** | Still down — `meta.generatedAt` is 19 Aug and the liveness canary is red at **34 days**. The **reader banner is silent as of 22 Sept, and has earned it**: T-bills 5 days (budget 10), CBK sovereign context 5 (14), USD/KES 1 trading day (4), CPI 22 (45), everything else well inside cadence. Fresh figures are not a working pipeline. The Data Health panel's "Pipeline last ran" row and `published-data-freshness.test.ts` are what still report the machinery, and they are the honest red — the one red test in a suite of 1,388. `meta.json` must never be hand-stamped to quiet them, which would not even work, since `readerNotice` stopped reading pipeline liveness in August. |
| **What the archive is missing** | **Rewritten 22 Sept from four primary CBK documents supplied by the owner** — the T-bill and bond results notices, the daily FX card, and the whole Weekly Bulletin of 18 September 2026. **T-bills** are at the **17 Sept** auction — 8.7837 / 8.9099 / 9.0566, issues 2700/091, 2674/182, 2629/364 DATED 21-09-2026, `nextAuctionDate` 24 Sept — reconciled on the column total (23,231.01 + 10,046.13 + 9,415.69 = 42,692.83M accepted against 28,000.00M offered) and corroborated independently by the bulletin's Table 4 (8.784 / 8.910 / 9.057). **`auction-results.json` now ends 16 Sept**, not 7 Sept: the re-opened FXD1/2019/020 (WAR 13.6105, market 13.6799, price 101.5472) and FXD1/2026/030 (WAR 14.2355, market 14.4684, price 93.2777), signed Robert Aloo, value dated 21/09/2026 — every column total reconciles and the bulletin's Table 5 carries the same two rates rounded (13.61, 14.24). Both bonds were already in `bonds.json` with **matching ISIN, coupon and maturity** (KE5000009984 / 12.873 / 2039-03-21 and KE8000006760 / 12.5 / 2056-03-13), which is the cross-check that makes the transcription checkable; their `ytmGross` and `ytmAsOf` were advanced to the 16 Sept auction. **Reserves** are **6.1 months, USD 15,088m at 17 Sept** — note the series is not monotonic (6.3 on 20 Aug, 6.2 on 27 Aug, 6.1 on 3 Sept, **6.3 on 10 Sept**, 6.1 on 17 Sept), so the "a figure that breaks its series is wrong" heuristic would have rejected a true reading here; the bulletin's own Table 2 is what settles it. **USD/KES is 129.49 at 21 Sept** from the daily card, consistent with the bulletin's 129.62 on 17 Sept and 129.59 weekly average. **`cbk-context.json` gained a second row**, gross domestic debt KSh 7.73tn at 11 Sept (Table 7), bonds 84.27 percent of securities. August CPI 6.6 was appended to `cpi-history.json`, which was 10 days from breaching its 75-day budget. The standing rule survives all of this: press outlets are unregistered in `licences.ts` and `licences.test.ts` fails the build on a source that does not resolve — it caught a prose `source` string written into `bonds.json` in this very session, which is why that field is a URL and the provenance lives in `sourceNote`. |
...
| **`expectations.json`** | New 21 Aug — CBK Market Perceptions Survey inflation expectations, four horizons, banks and non-banks separate. Opinions, not measurements: fenced in the UI, excluded from `macroRegime` by test, and budgeted at **100** days (raised from 75 on 24 Sept: the budget must span fieldwork to the NEXT report — a 57-day MPC gap plus ~34 days to publication — or it cries wolf every cycle). The next MPC date is `public/data/mpc-next.json`, hand-updated from CBK's 'Next MPC Meeting' notice; past it, `/macro/` falls back to a 60-day estimate and labels it so. |
| **Bid guidance is CALIBRATED now — and the obvious diagnosis was wrong** | **Measured 22 Sept.** The raw method scored range 54%, middle-half **22%** (a band claiming 50%), median |err| 0.70pp. It now scores **66% / 53% / 0.67**, from two measured adjustments in `guidanceCalibration`: subtract the median error of the last `CORRECTION_WINDOW` (**12**) scored auctions, and widen the middle band `BAND_SCALE` (**1.75×**). Both constants sit mid-plateau — every window from 8 to 24 improves every metric — so neither is a tuned spike; the tables are in the doc comments. **The tempting diagnosis was tested and is FALSE:** the lag is NOT caused by the full-archive fallback reaching into the 17-18pp era. Capping the lookback fixes nothing — a one-year window still measures **+0.53pp** — because the market moves faster than any trailing sample follows. Do not 'fix' this by shortening `RECENCY_LADDER`; it was tried, measured, and discarded. **The error series genuinely turns in late May 2026** (consistently +0.4 to +1.28 before, mostly -0.2 to -0.7 after), which is why a 24-claim window reads +0.42 while a 12-claim reads -0.29 — the same archive, opposite signs. `recentBiasPp`, added hours earlier to WARN about the lag, was retired the same day: correcting a bias beats describing it, and two windows disagreeing in sign on one page is worse than either alone. `backtest(..., {calibrate:true})` replays the corrected method point-in-time with a rolling shift built only from errors predating each auction; a test truncates the archive and asserts the forecast is unchanged, because a correction fitted to the whole archive would be lookahead wearing the clothes of an improvement. IFB errors are the outliers (+3.64pp on 17 Aug), which is why the shift is a MEDIAN. |
| **Rollback target: capture it BEFORE, because after is impossible** | Production is deploy **`6ab68edd86c5ec00083f33e3`**, commit `be55774` (#310), published 2026-09-25T15:11:09Z, rollback target `6ab587e0` (#307/#308) — `state: ready`, secret scan 567 files / 0 matches, `available_functions` `[track]`, `edge_functions_present: true`, `plugin_state: success`. (#309's own deploy was superseded before capture.) Recorded in full in [`docs/RUNBOOK-DEPLOY.md`](docs/RUNBOOK-DEPLOY.md). **The runbook's "write the current deploy ID down first" is not a nicety.** Asked for a rollback target AFTER the deploy published, a session could not reconstruct the previous one at all: the Netlify MCP exposes only `get-deploy` and `get-deploy-for-site`, both needing an ID you already hold, and there is no list-deploys; no GitHub tool lists check runs for an arbitrary commit (only a PR head), so the Netlify check's `details_url` carrying the ID is unreachable for a commit already on `main`; and `api.netlify.com` is egress-blocked, ruling out the REST API and the CLI. The **Netlify MCP does work** though — it authenticates server-side, same as Vercel's — so `get-projects` plus `get-deploy-for-site` is the way to verify a deploy from a session, and it is how the figures above were read rather than assumed. |
| **Lazy-loading Dexie does NOT speed pages up — tested 24 Sept, discarded** | The shared chunk every route runs on load is 97 KB and is almost entirely Dexie (96,136 B minified), pulled in because `AppInit` (root layout) imports every store and every store imported `@/lib/db`. The obvious fix — `import('./db')` behind a `getDb()` — was built across 8 files, type-checked, guarded by a mutation-checked test, and **measured with no gain**: interleaved Lighthouse mobile, 4 runs each, `main` perf 64 / TBT 533ms / LCP 6.66s against lazy 62 / 599ms / 6.59s — noise. The reason: `AppInit` asks the stores for data on mount, so the lazy chunk loads in the same window anyway; the work moves, it does not disappear. A real gain would need the stores to stop reading IndexedDB on pages that never render stored data — an architecture change, not an import change. **Two measurement traps cost this session a false result first:** `npx serve` 301-redirects `/` and `/index.html` alike, so compare two builds under the same server rather than against a remembered number; and a first reading of "TBT 960 → 560" compared runs from different setups and reported a 40% win that did not exist. Lighthouse mobile here swings 61–69 on an unchanged build, so anything under ~5 points needs ≥4 interleaved runs before it means anything. |
| **The archive is replayed IN THE BROWSER — count your passes** | `/auctions/` replays all ~390 prints client-side, and the calibration work made that three separate replays: **297ms** of blocked main thread on a dev machine, for one page, on every render — and this audience is on mid-range Android, where it is several times worse. `backtestPair` does it in **one** traversal (**121ms**, faster than the single raw backtest the page ran before calibration existed) by handing out the raw results the calibrated pass already builds, deriving the calibration from those rather than replaying, and priming a WeakMap memo that `BidAssistant` — same page, different component — then reads in **0ms**. A React `useMemo` only dedupes within its own component, which is why two components asking the same question paid twice. The memo is keyed on array IDENTITY, which is correct because the store hands every consumer the same arrays, and a test asserts a different archive does not collide. Equivalence is tested, not assumed: one pass must return exactly what three returned, or the speed is worth nothing. **This was found by noticing the e2e suite had gone slow**, not by a profiler — if that suite starts dragging after a change to a lib the pages compute from, measure before assuming it is the harness. |
| **Vercel's "Account is blocked" is a BILLING SUSPENSION — confirmed 24 Sept** | Creating a Vercel Sandbox (to run the scrapers on a VM with open egress, the one compute surface with real network that a session could drive) returned **402 `resource_creation_blocked`: "Your account has been suspended. To reactivate your subscription, add a valid payment method."** That is the root of the red `Vercel` status on every PR, and it closes Vercel as a refresh route: sandboxes, deployments and builds are all refused. So every compute surface with egress is now accounted for — GitHub Actions (billing), Vercel (billing suspension), this sandbox (egress proxy). The remaining one is a Netlify-side refresh (a scheduled function writing to Netlify Blobs, which the site already depends on); that is an architecture change that spends the metered build pool, so it is scoped, not started. No GitHub tool manages app installations, so detaching the Vercel app from this repo is not possible from a session either. |
| **Vercel: the API is READABLE but cannot unlink — checked properly on 22 Sept** | This file used to call the Vercel remedy 'dashboard-only'. Half wrong: the Vercel **MCP** works from a session (it authenticates server-side) and reads real state — `mwangaza-yield` `prj_sSLiX7dCSiqTjh0YX9zVzOZbAqKT`, `live: false`, latest deployment `BLOCKED`, and `get_git_deployment_context` shows the link as `github:PapaDanico/Mwangaza-Yield` alongside 12 sibling projects on other repos. **But no exposed tool can REMOVE that link.** `update_project` has no `link` or `gitRepository` field (it can create one via `create_project`/`create_git_project`, never delete); there is no unlink or delete-project tool; and the real endpoint, `DELETE /v9/projects/{id}/link`, is not surfaced. A direct call is doubly blocked: `api.vercel.com` returns **000** through the egress proxy, and there is no Vercel token in the environment, no `~/.vercel`, no `.vercel/project.json`. **The lesson is the one this file keeps relearning:** a session confirmed the API was reachable, then offered to do a specific operation it had never checked was available — reachable is not the same as capable, and the offer was made in a PR comment before the check. Verify the OPERATION, not just the host. The remedy stays the owner's UI: GitHub → Settings → Integrations → Vercel → remove this repo from its access (scoped per-repo, so the other 12 projects are untouched), or the Vercel dashboard's Git disconnect. The status is cosmetic — it blocks no merge and reaches no reader. |
| **`update-predictions.mjs` needs no network — run it** | It reads `auctions.json`, `auction-results.json` and `bonds.json` plus `dist/engine`, and touches nothing remote. It lives inside `refresh-data.mjs`, which DOES need network, so it had not run since 19 Aug and the ledger silently stopped recording. Run `npm run build:engine && node scripts/update-predictions.mjs` after any auction edit. It refuses retroactive forecasts — `recordPredictions` skips `auctionDate <= today` — so a gap in the ledger is permanent and honest, which is why the 2 and 16 Sept bond auctions have no prediction and never will. It also only forecasts BONDS: a T-bill issue code like `2701/091` has no third `/` segment, so `parseFloat` gives NaN and the row is skipped by design. |
| **Debt/GDP WAS computable all along — the IMF series sat unread for a month** | **Corrected 24 Sept; this row said the opposite for two days.** It claimed the ratio could not be computed because "nominal GDP in KSh ... nothing here holds and no reachable source provides". False: `imf-outlook.json` has carried IMF WEO general government gross debt (`GGXWDG_NGDP`) since 29 August, licensed and shipped — and **no file in `src/` read it**. The owner found it by sending a Statista link; Statista republishes the same WEO series. The session that wrote the old row never opened the file, which is the exact failure this file's opening section warns about. Now wired: `macro.json` carries `DEBT_TO_GDP` = **67.338**, the **2024 outturn** (not the 2025 estimate 69.33 or 2026 projection 71.58 — a verdict rests on a measurement), dated by VINTAGE 2026-04-30 like the GDP row, budget 240 days in both `indicator-freshness.ts` and `healthcheck.py`. Every held value was corroborated against an independent report: 2023 73.41 (press "73.4% peak"), 2025 69.33 (Statista, exact), 2026 71.58 and 2027 72.39 (IMF April 2026 Regional Economic Outlook, "71.6% ... 72.4%"). April 2026 is the current vintage. `macroRegime` gains a **"Moderate debt pressure"** tag (above the IMF's 55% ceiling) and its label stays "Mixed signals" — the figure made it more cautious, not more confident. `sovereignGaps` takes `heldElsewhere` so the World Bank's empty series no longer produces a "we do not have this" notice beside a page that shows it. `www.imf.org`, `data.imf.org` and `api.imf.org` are all egress-blocked (`connect_rejected`), so verification was by corroboration, not by fetch. **The Fiscal Health card renders as of 24 Sept**: `INTEREST_TO_REVENUE` = **41.508**, computed from the owner-supplied Q4 FY2025/26 QEBR (interest 1,074.4bn / ordinary revenue 2,588.4bn; the QEBR prints no ratio). A search-corroborated 42.7 went live for an hour first and was WRONG: press had foreign interest at 205bn against the document's 211.8bn, and a ratio the primary figures do not reproduce. Three internal cross-checks passed on it anyway — reconciliation shows a number is plausible, not that it is right. `cbk-context.json` carries the same 41.5 as `Interest / government revenue` (asOf 2026-06-30), which outranks the World Bank's 24.28 (2023) by `mergeSovereign`'s newest-wins rule, so the page no longer shows two contradictory values. Current account (-2.5%, year to May) and external debt (US$ 43.9bn, June) were added from the same QEBR. **The other IMF series are now read too** (`imf-context.ts`, merged into the Sovereign Context panel since 24 Sept): growth outlook, inflation outlook, fiscal balance, nominal GDP and GDP per capita. Fiscal balance is outranked by the Treasury's FY2025/26 outturn (-6.8%) under newest-wins. Only the IMF current-account series is unused, deliberately — the Q4 QEBR's -2.5% (year to May 2026) is newer. |
