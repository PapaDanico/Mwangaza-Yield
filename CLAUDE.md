...
| **Debt/GDP still cannot be computed, and that is the right answer** | The CBK Weekly Bulletin gives public debt as a LEVEL (KSh 13,011.89bn, June-26, Table 10) and gross domestic debt (KSh 7,726.50bn, 11 Sept, Table 7). Both are now context rows. The RATIO needs nominal GDP in KSh, which nothing here holds and no reachable source provides, so `macroRegime`'s `debtToGDP` stays `null` and contributes nothing to the verdict — verified 22 Sept by running it: with CBR 8.75 and CPI 6.6 the real rate is +2.15pp and the label is "Mixed signals" on a neutral stance, built from the two tags it actually has. `sovereignGaps` names the gap and points at the Treasury bulletin; that text was checked and is still accurate. Do not estimate the ratio to fill the hole. |
